...
function MenuItemList({ categories, selectedCategory, onEdit, onDelete }) {
      ...
      {items.map((item) => (
        <div key={item.id} className={styles.item}>
          <div className={styles.itemHeader}>
            <div>
              <h3 className={styles.itemName}>{item.nameAr || item.nameIt || item.name}</h3>
              {item.nameHe && <p className={styles.itemNameIt}>{item.nameHe}</p>}
              {item.name && <p className={styles.itemNameEn}>EN: {item.name}</p>}
            </div>
            <div className={styles.itemPrice}>₪{item.price.toFixed(2)}</div>
          </div>

          {(item.descriptionAr || item.description) && (
            <p className={styles.itemDescription}>{item.descriptionAr || item.description}</p>
          )}
