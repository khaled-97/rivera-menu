      ...
      {categories.map((category) => (
        <div key={category.id} className={styles.category}>
          <div className={styles.categoryHeader}>
            <div>
              <h3 className={styles.categoryName}>{category.nameAr || category.nameIt || category.name}</h3>
              {category.nameHe && (
                <p className={styles.categoryNameIt}>{category.nameHe}</p>
              )}
              {category.name && (
                <p className={styles.categoryNameEn}>EN: {category.name}</p>
              )}
            </div>
          ...
          {(category.descriptionAr || category.description) && (
            <p className={styles.categoryDescription}>{category.descriptionAr || category.description}</p>
          )}
            ...
            <div className={styles.items}>
              ...
              <ul className={styles.itemsList}>
                {category.items.map((item) => (
                  <li key={item.id}>
                    {item.nameAr || item.nameIt || item.name} <span className={styles.itemPrice}>₪{item.price}</span>
                  </li>
                ))}
              </ul>
            </div>
